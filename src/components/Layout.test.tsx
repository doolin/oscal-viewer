...
describe("<Layout /> SSP count badge (port upstream #59)", () => {
  ...
  it("desktop: SSP tab carries a tooltip listing every loaded SSP fileName", () => {
    ...
    expect(title).toContain("Main SSP: current.json");
    expect(title).toContain("Leveraged SSP 1: provider1.json");
    expect(title).toContain("Leveraged SSP 2: provider2.json");
  });
