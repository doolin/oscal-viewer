...
| [#70](https://github.com/EasyDynamics/oscal-viewer/pull/70) | `contrib/cookie-party-helper-coverage` | 18 tests against `partyDisplayName` + `sanitizedAnalyticsPath` + `viewerAnalyticsPath` | open 2026-05-24 (rebased 2026-05-28) | Rebased onto `806f015a` to pick up #69 fork-PR conditional; Azure check now skipped cleanly. 2 files, +122/-0. |
| [#71](https://github.com/EasyDynamics/oscal-viewer/pull/71) | `contrib/catalog-sort-index-coverage` | 14 unit tests for `useCatalogSortIndex` (sort-id index + comparator) including one explicit BUG lock-in for case-asymmetric lookup | open 2026-05-28 | Recovered from local PR #102 (port of upstream #57) at commit `21f84c6f`. Uses real `OscalProvider` + `Seed` helper, not `vi.mock`. Sets up natural two-PR fix narrative for the case-asymmetry bug. 1 file, +293/-0. |
